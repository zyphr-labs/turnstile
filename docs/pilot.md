...
The next evidence needed is a consented daily-use trial measuring completed tasks, unnecessary confirmations, provider failures, and missed violations. The [roadmap](../ROADMAP.md) prioritizes Pi and OpenCode daily operation, real Claude Code validation, and a Codex integration, followed by Kimi Code, Prime Agent, and ZCode. Approval recovery on hosts that block review and confirmed execution receipts remain separate work. Existing Gemini CLI and Cursor adapters still need real-host validation. The alpha does not provide OS containment, unattended fleet operation, or a published npm package.
